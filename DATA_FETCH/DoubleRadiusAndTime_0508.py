    ...
    with connect.cursor() as curosr:
        ...
        for l_c in list_case:
            ...
            for l_f in list_force:
                sql="select time_s from pedestrian_fix_20180506.danger_double_for_meet WHERE case_s='%s' and density='%s' and r_c='%s' and r_o='%s' and force_s='%s'"
                curosr.execute(sql, [l_c, list_density[0], list_radius[2],list_radius_ob[0],l_f ])
