...
try:
    with connect.cursor() as curosr:
        ...
        crl_14=[]
        for l_c in lsit_case:
            for l_d in list_density:
                for l_r in list_radius_core:
                    for l_s in list_steps:
                        single_rol=[]
                        sql='select time_s,v  from pedestrian_fix_20180506.main_speed where case_s=%s and density=%s and radius=%s and step_s=%s '
                        curosr.execute(sql,[l_c,l_d,l_r,l_s])
